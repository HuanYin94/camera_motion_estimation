...
for n = 1 : 1 : 3
    ...
    beta = 0.5;
    refImage = imresize(imread('0000001465l.jpg'), 1/s);
    refImager = imresize(imread('0000001465r.jpg'), 1/s);
    curImage = imresize(imread('0000001455.jpg'), 1/s);

    grayImageRef = rgb2gray(refImage);
    grayImageCur = rgb2gray(curImage);

    edgeDetectedRef = edge(grayImageRef, 'canny', 0.05*n);
    edgeDetectedCur = edge(grayImageCur, 'canny', 0.05*n);
    [currex,currey] = find(edgeDetectedCur>0);
...
%     cx = 318.6; cy = 255.3; fx = 517.3; fy = 516.5;
    cx = 335.963; cy = 190.625; fx = 351.498; fy = 351.498;
    cx = cx/s; cy = cy/s; fx = fx/s; fy = fy/s;
%     depthOfRef = double(imresize(imread('0000001465.png'),1/s,'nearest'));
    D = disparity(rgb2gray(refImage),rgb2gray(refImager));
    depthOfRef = double(fx*120./D);
    depthOfRef(depthOfRef<0 | depthOfRef>30000) = 0;
    ...
    %     deltaRT = [deltaRotate, deltaTranslate; 0, 0, 0, 1];  
        ...
        plot(currey, currex,'r.')
        hold off;

        drawnow
        
        %break?
        if norm( deltaRT - eye(4), 2 ) < 1e-4 %* n.^(0.5)
            s = s /2;
